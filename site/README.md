...
Portfolio website for Inturious Labs. This folder is the Root Directory of the Vercel
project `inturious`; run every command below from inside `site/`.

## Deployment

- **Website:** https://inturious.com
- **Hosting:** Vercel (via git integration — auto-deploys on push to `main` when a commit touches `site/`)
...
held by a payment processor. It is its own Vercel project and deploys independently.
...
Dollar amounts are converted using rates from `api.inturious.com/api/tips/rates`
(source in `../api/`), which proxies CoinGecko so readers never contact a third party. If that is unreachable the
...
Vercel runs `scripts/build-decks.sh` from this folder, which builds each deck into `decks/<deck-slug>/` (alongside `src/`) and serves it at `https://inturious.com/decks/<deck-slug>/`. Drafts (folders prefixed with `_`) are skipped. The sitemap auto-discovers the new deck.
